    ...
    <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
      <article className="rounded-3xl border border-white/10 bg-slate-950/70 p-8 shadow-2xl shadow-slate-950/20">
        <div className="flex flex-wrap items-center gap-3">
          <span
            className={`rounded-full border px-3 py-1 text-xs font-medium ${difficultyStyles[problem.difficulty]}`}
          >
            {problem.difficulty}
          </span>
      ...
      <article className="rounded-3xl border border-white/10 bg-slate-950/70 p-6 shadow-2xl shadow-cyan-950/10">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Submit code through the sandboxed judge, then compare accepted
              performance on the Phase 4 leaderboard.
            </p>
          </div>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-slate-400">
            Runs are authenticated and executed against the full test suite, not
            just the sample cases.
          </p>
          <button
            type="button"
              ...
              try {
                ...
                setExecutionError(
                  error instanceof Error
                    ? error.message
                    : "Unable to execute this submission.",
                );
              } finally {
