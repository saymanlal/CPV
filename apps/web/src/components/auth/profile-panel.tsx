...
import { clearAuthToken, fetchProfile, getStoredAuthToken } from '../../lib/auth';
...
export const ProfilePanel = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setState({ status: 'error', message: 'You are not signed in yet. Create an account or log in to access your profile.' });
      return;
    }
    ...
    void fetchProfile(webEnv.NEXT_PUBLIC_API_BASE_URL, token)
      ...
      .catch((error) => {
        clearAuthToken();
        setState({ status: 'error', message: error instanceof Error ? error.message : 'Unable to load your profile.' });
      });
  }, []);

  if (state.status === 'loading') {
    return <Card title="Loading profile" body="Validating your JWT and fetching your Phase 1 profile…" />;
  }
  ...
  if (state.status === 'error') {
    return (
      <Card
        ...
        body={state.message}
        footer={<Link href="/auth" className="text-cyan-300 hover:text-cyan-200">Go to auth</Link>}
      />
    );
  }
    ...
    >
      ...
      >
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <Stat label="Created" value={new Date(user.createdAt).toLocaleDateString()} />
        </div>
      </Card>

      <Card title="What Phase 1 unlocks" body="Stable identity is now in place for the competitive platform roadmap.">
