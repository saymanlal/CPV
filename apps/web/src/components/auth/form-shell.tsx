...
type AuthFieldErrors = Partial<Record<'email' | 'username' | 'password', string>>;
type AuthFormValues = {
  email: string;
  password: string;
  username?: string;
...
export const FormShell = () => {
  ...
  const [values, setValues] = useState<Record<AuthMode, AuthFormValues>>(initialValues);
  ...
  const schema = useMemo(() => (mode === 'register' ? registerInputSchema : loginInputSchema), [mode]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!parsed.success) {
      const nextErrors = (parsed.error.issues as ValidationIssue[]).reduce<AuthFieldErrors>((accumulator, issue) => {
    ...
    try {
      ...
      setServerError(error instanceof Error ? error.message : 'Unable to complete the request.');
    } finally {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-slate-950/70 p-8 shadow-2xl shadow-cyan-950/20 backdrop-blur">
      ...
      <form className="mt-8 space-y-5" onSubmit={handleSubmit} noValidate>
        ...
        >
          {isSubmitting ? 'Please wait…' : mode === 'register' ? 'Create CPV account' : 'Enter arena'}
        </button>
      </form>

      <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm leading-6 text-slate-300">
        <p className="font-medium text-white">Phase 1 foundation</p>
        <p className="mt-1">
          Registration, login, JWT session validation, and the initial competitive rating profile are now available.
        </p>
        <p className="mt-3 text-slate-400">
          After authentication you&apos;ll land on a protected profile page that proves token validation is working end-to-end.
        </p>
      </div>
...
const Field = ({ error, label, name, onChange, placeholder, type, value }: FieldProps) => {
  return (
